 ...
 * readings and the fauna log. Written by the hour after BACKUP_HOUR by
 * the vessel's clock, before the hour takes its lease, so it never holds
 * the round up and the round never holds it up.
 *
...
/** The day (YYYY-MM-DD) and the hour (0-23) where the vessel is. The vessel's
 *  clock decides the day (vesselNow, from the vessel file): the tick is UTC. */
export type VesselDay = { day: string; hour: number };
...
export function backupDue(record: { day?: string | null } | null | undefined, there: VesselDay, afterHour: number): boolean {
  return there.hour >= afterHour && (record?.day ?? null) !== there.day;
}
...
async function backupBody(now: number, day: string): Promise<{ text: string; rev: number | null; counts: Record<string, number> }> {
  ...
  const text =
    '{"portal":' + JSON.stringify(vessel.slug) + ',"backupVersion":1' +
