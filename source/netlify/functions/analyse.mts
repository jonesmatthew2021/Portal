...
const OPMS_WORKER_PATH = "/.netlify/functions/opms-run";
const SHIFT_WORKER_PATH = "/.netlify/functions/shift-run";
const MATRIX_READ_WORKER_PATH = "/.netlify/functions/matrix-read-run";
const MATRIX_CHECK_WORKER_PATH = "/.netlify/functions/matrix-check-run";
 ...
 */
async function startWorker(req: Request, path: string, jobId: string) {
  ...
  try {
    ...
    // Anything but an answer in the two hundreds means the work was never picked
    // up, and a job left sitting at "running" would have the portal asking after
    // an answer that is never coming.
    return res.ok;
  } catch {
    // The site being unreachable from inside itself is the same thing as a
    // refusal as far as the run is concerned: nobody has the job.
    return false;
  }
}
