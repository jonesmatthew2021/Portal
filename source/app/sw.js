/* The portal's service worker: keeps the last-loaded portal readable when
   ...
   kept answers and the page across from an earlier build's cache (and
   only from one of those) so there is never a moment with nothing to
   read. The install itself keeps only the build's own files. A worker left
...
/* Counts the forgets. A copy fetched before a forget must never be kept
   after it: a sign-out deletes the cache and sends /logout on, and until
   the server's 303 comes back the cookie is still the last person's - a
   poll already out, or any request answered in that round trip, comes
   back 200 as them. Every forget bumps the era; a keep remembers the era
   its fetch went out in and drops the answer if a forget came between. */
let era = 0;
...
const forgetKept = async (cache) => {
  era++;
  await Promise.all([...KEPT_APIS, "/"].map((k) => cache.delete(k)));
};
...
/* What is kept at install is a head start, not a condition of installing:
   ...
   Only the build's own files are kept here: React, React DOM and the
   fonts. Neither who is signed in nor the page. An install runs on
   whatever cookie the device holds at that instant, and the browser finds
   a deploy on a navigation this worker handles - the sign-in POST and the
   sign-out included - so the new build installed while the old worker was
   still sending the code or the sign-out on: asked then, /api/me answered
   the last person, whose cookie the 303 had not yet replaced or revoked,
   and the page came as theirs, and both were kept in the new cache, which
   nothing ever forgot. The first build's head start on /api/me and on the
   page is the page's to give: once this worker takes control the page
   asks for both again, from a boot the server has already answered as the
   cookie's person, and those answers are kept the ordinary way. On a
   deploy the person's copies come across from the earlier build's cache
   on taking over (carry). */
self.addEventListener("install", (event) => {
  event.waitUntil((async () => {
    const cache = await openCache();
    if (cache) await Promise.all(VENDOR.map((key) => fetchAndKeep(cache, "vendor", key)));
    await self.skipWaiting();
  })());
});

/* The kept answers and the page are the crew's, not a build's code: they
   come across from an earlier build's cache, so a deploy does not leave a
   phone with nothing to read until its next poll or its next opening
   (the install no longer fetches the page; the next navigation with the
   link up replaces the carried one). Only a stamped copy comes (anything
   else is not one of this worker's), what this build has already kept
   wins, and nothing comes from a cache whose /api/me is a different
   person from the one this build has already kept - the last person's
   document must not land in the next person's cache. */
...
/* The sign-out and the sign-in's completing request (forgetsBefore) are
   ...
   Forgotten again once the server has answered. Until the 303 is back
   the cookie is still the last person's and their session not yet
   revoked or replaced, so anything this worker answered in that round
   trip - another tab's navigation to the page, a boot's /api/me, a poll
   - came back 200 as them, opened the cache again and was kept there,
   and nothing forgot it: a signed-out device booted offline as the
   person who had signed out. The second forget takes those, and the era
...
async function networkFirst(event, kind) {
  ...
  // The era this fetch goes out in: a forget between now and the keep
  // means the answer is the last person's, and it is dropped.
  const began = era;
  ...
  event.waitUntil(fromNetwork.then((answer) => keep(cache, kind, key, answer.clone(), began)).catch(() => {}));
